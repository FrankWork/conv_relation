...
def raw_dataset(data_file):
  '''load dataset from text file

  Args: 
    data_file: filename without path
  
  Returns:
    A list of `Raw_Example`s

  Example:
    for e in raw_dataset(file):
      label = e['label'] 
      entity1 = e['entity1']
      entity2 = e['entity2']
      sentence = e['sentence] # a list of tokens
      entity1.first # start position of entity1
      entity1.last  # end position of entity2
  '''
  data = []
...
class SequenceWrapper(object):
  ...
  def __init__(self, multivalent_tokens=False):
    ...
    self._cntxt = self._seq.context.feature
    self._timesteps = []
    self._multivalent_tokens = multivalent_tokens
  ...
  def add_context(self, raw_example):
    self._cntxt['entity1.first'].int64_list.value.append(raw_example['entity1'].frist)
    self._cntxt['entity1.last'].int64_list.value.append(raw_example['entity1'].last)
    self._cntxt['entity2.first'].int64_list.value.append(raw_example['entity2'].frist)
    self._cntxt['entity2.last'].int64_list.value.append(raw_example['entity2'].last)
    self._cntxt['label'].int64_list.value.append(raw_example['label'])
